...
///////////////////////////////////////////////TCPDUMP///////////////////////////
#define MAX_TCPDUMP_NUM 100
#define MAX_TCPDUMP_TIMEOUT 24*60*60
#define TCPDUMP_OLDEST_CMD "ps -eo pid,lstart,cmd|grep tcpdump|grep auto|grep -v grep|awk '{print $1,$2,$3,$4,$5,$6}'"
#define TCPDUMP_NUM_CMD "ps -ef|grep tcpdump|grep auto|grep -v grep|wc -l"
#define TCPDUMP_START_CMD "tcpdump -i eth0 -s 256 -c 10000 -p %s and \\(port %d or port %d\\) and \\(src host %s or src host %s\\) and \\(dst host %s or dst host %s\\) -w %s &"

static int tcpdump_datetime2sec(char * datetime)
{
	char datetime_cmd[1024];
	char tmp[1024];
	FILE *pp;
	if((SnortSnprintf(datetime_cmd, 1024,
			       "date -d '%s' +%%s",
					datetime
...
static int tcpdump_current_sec()
{
	struct timeval tv;
...
static void tcpdump_ip2addr(char* addr , unsigned long ip)
{
	struct in_addr ina;
	ina.s_addr= htonl(ip);
...
static void tcpdump_kill(int pid)
{
	char cmd[1024];
	FILE *pp;
	char tmp[1024];
	if((SnortSnprintf(cmd, 1024,
			       "kill %d",
					pid
			       )) != SNORT_SNPRINTF_SUCCESS)
	{
...
static int tcpdump_oldest(int* pid , char* re_datetime)
{

	char result[1024],*tmp;
	int olddatetime_sec,datetime_sec;
	char oldpid[32]={0} , olddatetime[64]={0} , *pid_str ,*datetime_str;
	FILE *pp = popen(TCPDUMP_OLDEST_CMD, "r"); 
    if (!pp)
	{
        return -1;
    }
    ...
    while (fgets(result, sizeof(result), pp) != NULL)
	{
		...
		*tmp='\0';
		...
		datetime_str=tmp;
		if(strlen(olddatetime)==0)
		{
			strcpy(oldpid , pid_str);
			strcpy(olddatetime , datetime_str);
		}
		olddatetime_sec=tcpdump_datetime2sec(olddatetime);
		datetime_sec=tcpdump_datetime2sec(datetime_str);
		if(-1==olddatetime_sec&&-1==datetime_sec)
		{
			pclose(pp);
			return -1;
		}
		if(olddatetime_sec>datetime_sec)
		{
			strcpy(oldpid , pid_str);
			strcpy(olddatetime , datetime_str);
		}
    ...
    pclose(pp);
	*pid=atoi(oldpid);
	strcpy(re_datetime , olddatetime);
	return 0;

}

static int tcpdump_timeout()
{
	int pid;
	char olddatetime[64];
	int olddatetime_sec , current_datetime_sec;

	current_datetime_sec=tcpdump_current_sec();
	
	if(-1==tcpdump_oldest(&pid , olddatetime))
	{
		return -1;
	}
	olddatetime_sec=tcpdump_datetime2sec(olddatetime);
	if(-1==olddatetime_sec)
	{
		return -1;
	}
	if(current_datetime_sec-olddatetime_sec>=MAX_TCPDUMP_TIMEOUT)
	{
		tcpdump_kill(pid);
	}
	return 0;
...
static int tcpdump_numout()
{
	char tmp[1024];
	int pid;
	char datetime[64];
	FILE *pp = popen(TCPDUMP_NUM_CMD, "r"); 
    if (!pp)
	{
        return -1;
    }
	...
	if(atoi(tmp)>=MAX_TCPDUMP_NUM)
	{
		if(-1==tcpdump_oldest(&pid , datetime))
		{
			pclose(pp);
			return -1;
		}
...
static int tcpdump_event(Packet *p, void *event, DatabaseData *data)
{
	char* dump_path="/tmp";
	char dump_filename[256]={0};
	char protocol[32]={0};
	char cmd[1024]={0};
	char saddr[16]={0},daddr[16]={0};
	int sport,dport;
	char date[32]={0};
	char time[32]={0};
	...
	if(-1==tcpdump_timeout())
	{
		return -1;
	}
	sscanf(data->timestampHolder , "%s %s" , date , time);
	if((SnortSnprintf(dump_filename, 1024,
			       "%s/auto_%s\\ %s_%d.pcap",
					dump_path,
					date,
					time,
	...
	if(p != NULL)
    {
    	tcpdump_ip2addr(saddr , (u_long)ntohl(p->iph->ip_src.s_addr));
		tcpdump_ip2addr(daddr , (u_long)ntohl(p->iph->ip_dst.s_addr));
		if((!p->frag_flag) && (IPH_IS_VALID(p)))
		{
		    switch(GET_IPH_PROTO(p))
		    {
				...
				{
					strcpy(protocol , "udp");
					sport=ntohs(p->udph->uh_sport);
					dport=ntohs(p->udph->uh_dport);
				}
				break;
			}
			
			if((SnortSnprintf(cmd , 1024,TCPDUMP_START_CMD , 
...
void Connect(DatabaseData * data)
{
	...
	mysql_set_character_set(data->m_sock, "utf8");
