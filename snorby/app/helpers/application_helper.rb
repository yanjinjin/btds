...
=end
  ...
  def rule_truncate(text, length = 30, truncate_string = "...")
    if text.nil?
      return ""
    end
    l = length - truncate_string.length
    if text.length > length
      return text[0...l] + truncate_string
    else
      return text
    end
  end
  ...
  def worker_status(show_image=false)
    ...
    validations = [{:check => Snorby::Jobs.sensor_cache?, :enabled => true, :desc => "Sensor Cache Job"},
                   {:check => Snorby::Jobs.sys_status?, :enabled => true, :desc => "Sys Status Job"},
