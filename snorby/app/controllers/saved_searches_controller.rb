...
class SavedSearchesController < ApplicationController
  ...
  def destroy
    ...
    respond_to do |format|
      if @search && @current_user.id == @search.user.id
        if @search.destroy
          format.html { redirect_to saved_searches_path, :flash => { :success => "搜索结果 `#{@search.title}` 删除成功." } }
        else
          format.html { redirect_to saved_searches_path, :flash => { :error => "搜索结果 `#{@search.title}` 删除失败" } }
        end
      else
