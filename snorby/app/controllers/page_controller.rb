...
class PageController < ApplicationController
  ...
  def dashboard
   ...
   # @range = params[:range].blank? ? 'last_24' : params[:range]
    @range = params[:range].blank? ? 'month' : params[:range]
  ...
  def force_cache
    str = `ps -ef | grep delayed_job | awk '{print $2}'`.to_s
    ids = str.split("\n")
    ids.each {|id| `kill -9 #{id}`}
    Snorby::Worker.init
 
    #sb = Snorby::Jobs::SensorCacheJob.new
    #sb.perform
  ...
  def results
    ...
    if params.has_key?(:search) && !params[:search].blank?
      ...
      if enabled_count == 0
        redirect_to :back, :flash => {:error => "搜索规则解析出错."}
