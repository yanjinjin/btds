...
class EventsController < ApplicationController
  ...
  def index
    ...
    order_in = " limit ? offset ?"
    order_out = ""
    if [:sid,:signature,:timestamp].include?(params[:sort])
      sort = "ev.#{params[:sort]}"
      order_in = "order by #{sort} #{params[:direction]} limit ? offset ?"
    else
      sort = "a.#{params[:sort]}"
      order_out = "order by #{sort} #{params[:direction]}"
    end

    sql = %{
      select e.sid, e.cid, e.signature,e.classification_id, e.users_count,
      e.notes_count, e.timestamp, e.user_id  
      from (select * from event ev  #{order_in}) e
      left outer join iphdr a on a.cid = e.cid  and a.sid = e.sid
    }

    sql = sql + order_out
    #puts sql
    # @events = Event.sorty(params, [sql], "select count(*) from event ev where UNIX_TIMESTAMP(ev.timestamp) >= UNIX_TIMESTAMP(CURRENT_DATE())")
    @events = Event.sorty(params, [sql], "select count(*) from event ev")
...
=end
  ...
  def download
    begin
      id = params[:id].split(",")
      event = Event.get(id[0],id[1])
      send_file "/tmp/auto_#{event.timestamp.strftime("%Y-%m-%d %H:%M:%S")}_#{event.cid}.pcap", :filename => "#{event.cid}.pcap"
    rescue Exception => e
      flash[:notice] = "文件不存在"
