...
class Event
  ...
  belongs_to :classification, :parent_key => :id, :child_key => :classification_id
  ...
  def detailed_json 
    ...
    event = {
      :sid => self.sid,
      :cid => self.cid,
      :classification_name => self.classification.classification_name,
  ...
  def self.to_json_since(time)
    ...
    events.each do |event|
      ...
      event = {
        :sid => event.sid,
        :cid => event.cid,
        :classification_name => event.classification.classification_name,
