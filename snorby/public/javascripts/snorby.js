      ...
      if (data.events[0].geoip) { klass = ' geoip' };
			...
			<li id='event_{{sid}}{{cid}}' class='event' style='display:none;' data-event-id='{{sid}}-{{cid}}' data-event-sid='{{sid}}' data-event-cid='{{cid}}'> \
				<div class='row"+klass+"'> \
					...
					<div class='click classification'?{{classification_name}}</div> \
		...
		input_style: function(){
			...
			$('div#form-actions button.cancel').live('click', function() {
				...
				return false;
                        ...
                        $('div#form-actions button.to_relevance').live('click', function() {
				window.location = '/relevances';
				return false;
			});

$('div#form-actions button.go_back').live('click', function(){hisstory.back();return false;});
