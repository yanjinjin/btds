module Snorby
  module Jobs
    module CacheHelper
	  ...
	  def has_sig_class_classification_ids?
		sql = %{
			SELECT * FROM snorby.sig_class_classification_ids;
		}
		!db_select(sql).empty?
	end
      ...
      def validate_cache_indexes
		...
		unless has_sig_class_classification_ids?
			puts "[~] Writing data to `sig_class_classification_ids` table"
			db_execute("insert into snorby.sig_class_classification_ids (sig_class_id,classification_id) values (21,1),(35,2),(36,3),(37,4);")
			#db_execute("insert into snorby.sig_class_classification_ids (sig_class_id,classification_id) select snorby.sig_class.sig_class_id,5 from snorby.sig_class where sig_class_id not in (21,35,36,37);")
		end
		
		puts "[~] Building trigger `sig_class_to_classification` "
		db_execute("create trigger sig_class_to_classification
					before insert on event
					for each row
					begin
					set new.classification_id = (select classification_id from sig_class_classification_ids c left join signature b on c.sig_class_id =b.sig_class_id where b.sig_id=new.signature);
					end;")

		puts "[~] Modify loginname to admin "
		db_execute ("update users set email='admin',timezone='PRC' where email='admin@admin.com';")
