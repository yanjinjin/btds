module Snorby
  ...
  class Worker < Struct.new(:action)
    ...
    def self.init
     @@mutex.lock
      if self.running?#auto start worker
=begin
        if !Snorby::Jobs::SysStatusJob.thread_status?
          if Snorby::Jobs.sys_status?
            Snorby::Jobs.sys_status.destroy!
          end
          Delayed::Job.enqueue(Snorby::Jobs::SysStatusJob.new(false), :priority => 10, :run_at => Time.now)
        end
=end
        if !Snorby::Jobs.sensor_cache?
          Delayed::Job.enqueue(Snorby::Jobs::SensorCacheJob.new(false), :priority => 1, :run_at => DateTime.now)
        end
      else
        self.start
=begin
...
=end
        if Snorby::Jobs.sensor_cache?
          Snorby::Jobs.sensor_cache.destroy!
        end
        Delayed::Job.enqueue(Snorby::Jobs::SensorCacheJob.new(false), :priority => 1, :run_at => DateTime.now)
=begin
        Delayed::Job.enqueue(Snorby::Jobs::SysStatusJob.new(false), :priority => 10, :run_at => Time.now)
