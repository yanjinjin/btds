...
static char *SCPerfGetLogFilename(ConfNode *stats)
{
    ...
    if (snprintf(log_filename, PATH_MAX, "%s/%04d-%02d-%02d-%02d-%s", log_dir,
...
static int SCPerfFileREcreate(SCPerfOPIfaceContext *sc_perf_op_ctx)
{
	char* file;
	...
	if ( (file = SCPerfGetLogFilename(NULL)) == NULL) {
        SCLogInfo("Error retrieving Perf Counter API output file path");
		return 0;
	}
	if(strcmp(sc_perf_op_ctx->file , file)!=0) {
		SCFree(sc_perf_op_ctx->file);
		sc_perf_op_ctx->file=file;
		 if (!SCPerfFileReopen(sc_perf_op_ctx)) {
            return 0;
   		 }
	}
	else{
		if(-1==access(sc_perf_op_ctx->file,0)){
			if (!SCPerfFileReopen(sc_perf_op_ctx)) {
				return 0;
			 }
...
static int SCPerfOutputCounterFileIface()
{
	...
	if (!SCPerfFileREcreate(sc_perf_op_ctx)) {
        /* Rotation failed, error already logged. */
        return 0;
    }
