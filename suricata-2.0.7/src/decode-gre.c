...
int DecodeGRE(ThreadVars *tv, DecodeThreadVars *dtv, Packet *p, uint8_t *pkt, uint16_t len, PacketQueue *pq)
{
    ...
    switch (GRE_GET_PROTO(p->greh))
    {
		...
		case 0x6558:
			  {
                if (pq != NULL) {
					if(len - header_len-ETHERNET_HEADER_LEN<=0) {
						return TM_ECODE_OK;
					}
                    Packet *tp = PacketTunnelPktSetup(tv, dtv, p, pkt + header_len+ETHERNET_HEADER_LEN,
                            len - header_len-ETHERNET_HEADER_LEN, IPPROTO_IP, pq);
                    if (tp != NULL) {
                        PKT_SET_SRC(tp, PKT_SRC_DECODER_GRE);
                        PacketEnqueue(pq,tp);
                    }
