...
int ConnectToRemote(char* host ,int port)
{  
	...
	json_sfd=socket(AF_INET,SOCK_DGRAM,0);
	if(json_sfd < 0) 
	{ 
		return -1;
	}

	connect(json_sfd, (struct sockaddr *)&address, sizeof(struct sockaddr_in));
	
	return 0;
}

int SendToRemote(char *buffer,int buffer_len){
	int offset=0;
	int total_len=0;
	while((offset=send(json_sfd,buffer+offset,buffer_len-offset ,0))<buffer_len){
		if(offset<0)
			return -1;
		total_len+=offset;
		offset=total_len;
